...
* Copyright (C) 2017, Filippovsky - изменения для работы через роутер на конкретный IP
*/
...
// Filippovsky comment: $broadcast_ip = preg_replace("/(\d+)\.(\d+)\.(\d+)\.(\d+)/", "$1.$2.$3.255", $local_ip);
$broadcast_ip = $options['ip']; // Filippovsky
...
// Filippovsky comment: socket_set_option($sock, SOL_SOCKET, SO_BROADCAST, 1); 
socket_set_option($sock, SOL_SOCKET, SO_BROADCAST, 0);  // Filippovsky
